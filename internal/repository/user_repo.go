...
func (u *user) GetUserByEmail(ctx context.Context, email string, password string) (models.User, error) {
	...
	default:
		...
		rights.name, rights.rules FROM users INNER JOIN rights ON email=$1 and password=$2 and rights.id=users.rights`, email, password)
