...
func (u *user) Create(ctx context.Context, user *models.User) (*models.User, error) {
	...
	default:
		...
		($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, user.Firstname, user.Lastname, user.Password, user.Email, user.Rights.UUID,
...
func (u *user) GetUserByEmail(ctx context.Context, email string) (models.User, error) {
	...
	default:
		...
		FROM users INNER JOIN rights email=$1 and rights.id = users.rights`, email)
		var user = models.User{}
		err := row.Scan(&user.ID, &user.Firstname, &user.Lastname, &user.Password, &user.Email, &user.Rights.UUID,
